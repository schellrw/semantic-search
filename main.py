...
def demonstrate_search(searcher, secondary_ranker):
    ...
    for query in demo_queries:
        ...
        # Check if this is hybrid or semantic search based on available columns
        is_hybrid = 'semantic_score' in results.columns and 'keyword_score' in results.columns
...
def main():
    ...
    if args.no_hybrid:
        print(f"Search mode: Semantic-only")
    else:
        print(f"Search mode: Hybrid (α={config.hybrid_alpha}, β={config.hybrid_beta})")
    ...
    try:
        ...
        # Check if model was auto-inferred and update display
        actual_model_dim = model.get_sentence_embedding_dimension()
        if actual_model_dim == 384:
            actual_model_name = "all-MiniLM-L6-v2"
        elif actual_model_dim == 768:
            actual_model_name = "all-mpnet-base-v2"
        else:
            actual_model_name = config.model_name
            
        if actual_model_name != config.model_name:
            print(f"Model auto-corrected: {config.model_name} → {actual_model_name}")
        
        searcher, secondary_ranker = setup_search_system(model, db, table, config, semantic_only=args.no_hybrid)
        
        if not args.skip_demo:
            demonstrate_search(searcher, secondary_ranker)
        
        if not args.skip_eval:
            metrics = run_evaluation(df_sample, searcher, config)
